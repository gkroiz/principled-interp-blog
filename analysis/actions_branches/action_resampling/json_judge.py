...
def read_rollout_from_json(rollout_dir: Path, run_id: str) -> Optional[tuple[str, dict]]:
    """
...
async def analyze_rollouts(
    ...
    messages_list = [
        create_analysis_prompt(r["content"], r["run_id"])
        for r in rollouts
    ]
    ...
    for r in results:
        ...
        if not r or all(v == "" for v in r.values()):
            analyses.append(RolloutAnalysis(
    ...
    print(f"🎯 REWARD HACK RATE: {summary_results['reward_hacked']}/{summary_results['total_runs']} ({100*summary_results['reward_hacked']/summary_results['total_runs']:.1f}%)")
    print(f"💡 HINT TERMINATION RATE: {summary_results['hint_terminated']}/{summary_results['total_runs']} ({100*summary_results['hint_terminated']/summary_results['total_runs']:.1f}%)")
